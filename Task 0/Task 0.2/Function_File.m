...
function eqbm_points = find_equilibrium_points(x1_dot, x2_dot)
  ...
  x2_dot == 0;
  ################## ADD YOUR CODE HERE ######################
  eqbm_points = solve(x1_dot==0,x2_dot==0);   %solve x1_dot and x2_dot to obtain equilibrium points 
  #disp(eqbm_points);   %display equilibrium points
  ############################################################  
endfunction
...
function jacobian_matrices = find_jacobian_matrices(eqbm_points, x1_dot, x2_dot)
  ...
  ################## ADD YOUR CODE HERE ######################
  F=@(x) double(x);    %convert symbolic values to double precision values
  jac=jacobian([x1_dot;x2_dot]);  %calculate jacobian 
  for i=1:length(solutions)
    tp=F(subs(jac,{x1,x2},solutions{i}));  %evaluate jacobian by substituting values of x1 and x2 obtained from equilibrium points
    jacobian_matrices(i)=tp;
  endfor
  #disp(jacobian_matrices)   %display the evaluated jacobian matrix
  ############################################################  
endfunction
...
function [eigen_values stability] = check_eigen_values(eqbm_pts, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    flag = 1;
    ################## ADD YOUR CODE HERE ######################
    Eig=eig(matrix);   %find eigen value for each entry in jacobian_matrices
    eigen_values(k)=Eig;   %store eigen values
    if(!all(real(Eig)<=0))   %if all real eigen values are negative flag will be set to 1 else 0
      flag=0;
    endif
    ############################################################
    if flag == 1
  ...
  #disp(eigen_values)   %display eigen values
  #disp(stability)      %display stability
endfunction
