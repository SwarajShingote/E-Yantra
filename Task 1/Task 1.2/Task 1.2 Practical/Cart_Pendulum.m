...
function dy = cart_pendulum_dynamics(y, m, M, L, g,  u)
  ...
  dy(1,1) = y(2);
  dy(2,1) = ( u- (m*g*cos_theta*sin_theta) - (m*L*sin_theta*(y(4)^2))) /(M+m*(sin_theta^2));
  dy(3,1) = y(4);
  dy(4,1) = (u*cos_theta - ((M+m)*g*sin_theta) - (m*L*cos_theta*sin_theta*(y(4)^2))) /(L*m*(sin_theta^2) + L*M);
endfunction
...
function [t,y] = pole_place_cart_pendulum(m, M, L, g, y_setpoint, y0)
  ...
  eigs =[-1.8,-1.2,-1.5,-1.9] ;                          ## Initialise desired eigenvalues
